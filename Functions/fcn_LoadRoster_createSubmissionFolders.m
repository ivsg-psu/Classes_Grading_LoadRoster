function fcn_LoadRoster_createSubmissionFolders(rootPath, rosterTable, varargin)
%% fcn_LoadRoster_createSubmissionFolders
%     fcn_LoadRoster_createSubmissionFolders creates one folder per student
%     at a selected rootPath location. The folders are created to match a
%     student's number.
%
% FORMAT:
%
...
% INPUTS:
%
%      rootPath: a string or character array that denotes the path to the
%      root location where directories will be created
%
...
% EXAMPLES:
%
%     See the script: script_test_fcn_LoadRoster_createSubmissionFolders
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check the rootPath to be sure it is a text style
        fcn_DebugTools_checkInputsToFunctions(rootPath, '_of_char_strings');
    end
end
...
Nstudents = height(rosterTable);
for ith_student = 1:Nstudents
    thisStudentNumber = rosterTable.('CanvasIDNumber')(ith_student);
    studentNumberString = sprintf('%.0d',thisStudentNumber);
    directoryToCreate = fullfile(rootPath,studentNumberString);
    fcn_DebugTools_makeDirectory(directoryToCreate,-1);
